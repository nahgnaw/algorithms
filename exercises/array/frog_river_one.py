...
def solution(X, A):
    earliest_leaf_falling_time = [-1] * (X + 1)
    earliest_leaf_falling_time[0] = 0
    
    for i in xrange(len(A)):
        if A[i] <= X and \
            (earliest_leaf_falling_time[A[i]] == -1 or earliest_leaf_falling_time[A[i]] > i):
            earliest_leaf_falling_time[A[i]] = i
            
    if -1 in earliest_leaf_falling_time:
        return -1
    
    return max(earliest_leaf_falling_time)
